...
const RoomListDelete = () => {
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            ...
            try {
                const response = await service.apiGetRoomListDeletedFilter(filters);
    ...
    const handleRecover = async (id) => {
        const result = await MySwal.fire({
            title: "Xác nhận khôi phục phòng trọ",
            text: `Bạn có chắc chắn muốn khôi phục phòng này không?`,
            icon: "warning",
        ...
        if (result.isConfirmed) {
            try {
                const response = await service.apiRecoverRoom(id);
                if (response.data.err === 0) {
                    if (response.data.msg.includes("Đã khôi phục phòng trọ")) {
                        toast.success(response.data.msg)
                        const roomUpdate = await service.apiGetRoomListDeletedFilter(filters);
                        setRooms(roomUpdate.data.result);
                    } else {
                        toast.error(response.data.msg)
                    }
                } else {
                    alert(`Không thể khôi phục phòng trọ: ${response.data.msg}`);
                }
            } catch (error) {
                alert(`Lỗi khi khôi phục phòng trọ: ${error.message}`);
            }
        }
    };
